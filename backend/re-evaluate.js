...
async function main() {
  ...
  for (const r of responses) {
    ...
    const studentAnswer = r.answer_json;
    ...
    if (["FILL_BLANK", "NUMERICAL", "ASSERTION_REASON"].includes(q.question_type)) {
      if (answerKey && answerKey.correct_option) {
        // It's an MCQ-based question!
        const isCorrect = studentAnswer && studentAnswer.selected_option === answerKey.correct_option;
        ...
        if (isCorrect !== r.is_correct) {
          ...
          const pMarks = r.test_attempt.test.positive_marks || (tq && tq.marks_override) || q.marks || 0;
          const nMarks = (r.test_attempt.test.negative_marks !== null ? r.test_attempt.test.negative_marks : q.negative_marks) || 0;
