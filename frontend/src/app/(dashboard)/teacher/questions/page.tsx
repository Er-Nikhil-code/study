...
const difficultyBadge = (d: string) => {
  ...
  return (
    <span
      className={`rounded-full border px-3 py-1 text-[10px] font-medium ${colors[d] || "border-zinc-500/20 bg-zinc-500/10 text-zinc-300"}`}
    >
      {d}
    </span>
  );
          ...
          {loading ? (
            ...
            <div className="divide-y divide-white/10">
              {questions.map((q) => {
                ...
                return (
                  <div key={q.id}>
                    <div className={`grid grid-cols-[180px_minmax(200px,3fr)_120px_100px_minmax(150px,2fr)_100px_140px] gap-3 px-5 py-4 text-sm items-center text-center ${q.approval_status === "DRAFT" || q.approval_status === "NEEDS_REVISION" ? "bg-white/[0.02]" : ""}`}>
                      <div className="font-mono text-[10px] text-zinc-500 break-all min-w-0" title={q.id}>
                        {q.id}
                      </div>
