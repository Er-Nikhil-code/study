        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
          {(user?.role === "ADMIN" || user?.role === "TEACHER") && (
            ...
            >
              ...
              <h3 className="text-sm font-bold text-white group-hover:text-red-400 transition-colors">Create Course</h3>
              <p className="text-xs text-zinc-500 text-center mt-1 group-hover:text-zinc-400">Add a new course.</p>
            </Panel>
          )}
          {(() => {
            ...
            return sortedCourses.map((course: any) => {
              ...
              return (
                <Panel key={course.id} className="relative flex flex-col p-0 transition-all hover:-translate-y-1 hover:border-red-500/40 overflow-hidden group bg-white/5 backdrop-blur-xl">
                  ...
                  <Link href={`/courses/${course.id}`} className="flex-1 flex flex-col p-4">
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xs font-semibold text-transparent bg-clip-text bg-gradient-to-r from-zinc-100 to-zinc-400 tracking-wide leading-relaxed drop-shadow-sm group-hover:from-white group-hover:to-zinc-300 transition-all">{course.name}</h3>
                        <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                          {canSeeCodeAndId && (
                            <span className="whitespace-nowrap shrink-0 text-[9px] uppercase tracking-wider font-semibold text-red-400 bg-red-400/10 border border-red-400/20 px-1.5 py-0.5 rounded">{course.code}</span>
                          )}
                          {!course.is_enrolled && (course.price > 0 || course.discount_price > 0) && (
                    ...
                    {!isStudent && (
                      <div className="mt-auto pt-3 border-t border-white/10 flex flex-col gap-2">
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="text-zinc-200 font-semibold bg-white/5 px-2 py-0.5 rounded">{course.sections?.length || 0} Sections</span>
                        </div>
                        {user?.role === "ADMIN" ? (
                  ...
                  {isStudent && (
                    <div className="px-4 pb-4 pt-1 z-10">
                      {course.is_enrolled ? (
                        ...
                        <Link href={`/courses/${course.id}`} className="block w-full rounded-lg bg-red-600 hover:bg-red-500 transition-all py-1.5 text-center text-xs font-bold text-white shadow shadow-red-500/20">
                          Enroll
                        </Link>
                      )}
                    </div>
